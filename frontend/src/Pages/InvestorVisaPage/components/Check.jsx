...
const MIN_REQUIRED = 204; // тыс. $ (~750K AED)
const BASE_VISA_FEE_AED = 4100;
const FAMILY_FEE_AED = 2800;
const AED_PER_USD = 3.6725;
const TYPICAL_YIELD = 0.072; // 7.2% NET по UAE rental market

export default function Check() {
  const isMobile = useIsMobile();
  const t = useTranslations("InvestorVisaPage.check");
  ...
  const totalFamilyAED =
    BASE_VISA_FEE_AED + Math.max(0, familySize - 1) * FAMILY_FEE_AED;
  const totalUSD = totalFamilyAED / AED_PER_USD;
  const monthlyRent = Math.round(((budget * 1000) * TYPICAL_YIELD) / 12);
  const duration = planType === "ready" ? "20–30" : "30–35";
        ...
        >
          <div>
            ...
            >
              {t("titleA")}{" "}
              <span
                style={{ fontStyle: "italic", color: "var(--silver-deep)" }}
              >
                {t("titleB")}
              </span>
            </h2>
...
function ResultCell({ label, value, accent, silver }) {
  const color = accent
    ? "oklch(0.55 0.13 145)"
    : silver
      ? "var(--silver-deep)"
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: 999,
            background:
              "linear-gradient(90deg, var(--silver-deep), var(--silver))",
          }}
        />
        <input
          type="range"
