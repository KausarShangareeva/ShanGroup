...
// Premium dark lead form (#0A0A0B always) с gold-акцентами.
export default function LeadForm() {
  const isMobile = useIsMobile();
  const t = useTranslations("AirbnbPage.leadForm");
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <FormBlockLabel>{formCfg.statusLabel}</FormBlockLabel>
              <div
                style={{
                  display: "grid",
              ...
              >
                {formCfg.statuses.map((g) => {
                  const active = form.status === g.id;
                  return (
                    <ToggleChip
                      key={g.id}
                      active={active}
                      onClick={() => setForm({ ...form, status: g.id })}
                    >
                      {g.l}
                    </ToggleChip>
                  );
