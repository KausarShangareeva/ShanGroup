...
// Расходные коэффициенты для NET-формулы Airbnb-юнита.
// Согласовано с экономикой из Economics.jsx.
const OTA_FEE = 0.15;
const DTCM_TAX = 0.035;
const SHAN_FEE = 0.18;
const CLEANING_YEAR = 380 * 12;
const OPS_RATE = 0.06;

export default function Calc() {
  const isMobile = useIsMobile();
  const t = useTranslations("AirbnbPage.calc");
  const controls = t.raw("controls");
  const result = t.raw("result");

  const [price, setPrice] = useState(450); // тыс. $
  const [adr, setAdr] = useState(220); // $ за ночь
  const [occupancy, setOccupancy] = useState(86);

  const grossAnnual = adr * 365 * (occupancy / 100);
  const ops = grossAnnual * OPS_RATE;
  const netAnnual =
    grossAnnual * (1 - OTA_FEE - DTCM_TAX - SHAN_FEE) - CLEANING_YEAR - ops;
  const yieldPct = ((netAnnual / (price * 1000)) * 100).toFixed(2);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <ResultCell
                label={result.netYieldLabel}
                value={`${yieldPct}%`}
                accent
              />
              <ResultCell
                label={result.grossYearLabel}
                value={`$${Math.round(grossAnnual / 1000)}K`}
              />
              <ResultCell
                label={result.netYearLabel}
